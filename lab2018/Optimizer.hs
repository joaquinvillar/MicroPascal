...
module Optimizer where
    ...
    reduceIt ((Assig name expr):xs) = [Assig name (reduce expr)] `debug` "reduceIt Assig"  ++ reduceIt xs `debug` "reduceIt Assig xs" 
    ...
    reduce (Binary Or (BoolLit False) expr) = reduceBasic(Binary Or (BoolLit False) (reduce expr)) --cambiar
    reduce (Binary Or expr (BoolLit False)) = reduceBasic(Binary Or (reduce expr) (BoolLit False)) --cambiar
    ...
    reduce (Binary And expr (BoolLit True)) = reduceBasic(Binary And (reduce expr) (BoolLit True)) --cambiar
    reduce (Binary And (BoolLit True) expr) = reduceBasic(Binary And (BoolLit True) (reduce expr)) --cambiar
    reduce (Unary Not (BoolLit True)) = BoolLit False
    reduce (Unary Not (BoolLit False)) = BoolLit True
    reduce (Unary Not expr) =  reduceBasic(Unary Not (reduce expr)) --cambiar 
    reduce (Binary Equ (IntLit x) (IntLit y)) = if x == y then BoolLit True else BoolLit False
    reduce (Binary Equ exp expd) = reduceBasic(Binary Equ (reduce exp) (reduce expd)) --cambiar
    reduce (Binary Less (IntLit x) (IntLit y)) = if x < y then BoolLit True else BoolLit False
    reduce (Binary Less exp expd) = reduceBasic(Binary Less (reduce exp) (reduce expd)) --cambiar
    reduce (Binary Plus (IntLit x) (IntLit y)) = IntLit (x + y)
    reduce (Binary Plus exp expd) = reduceBasic(Binary Plus (reduce exp) (reduce expd)) --cambiar
    reduce (Binary Minus (IntLit x) (IntLit y)) = IntLit (x - y)
    reduce (Binary Minus exp expd) = reduceBasic(Binary Minus (reduce exp) (reduce expd)) --cambiar
    reduce (Binary Mult (IntLit x) (IntLit y)) = IntLit (x * y)
    reduce (Binary Mult exp expd) = reduceBasic(Binary Mult (reduce exp) (reduce expd)) --cambiar
